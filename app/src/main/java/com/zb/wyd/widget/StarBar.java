...
public class StarBar extends View
{
    ...
    private void init(Context context, AttributeSet attrs)
    {
        ...
        this.starFillBitmap = drawableToBitmap(mTypedArray.getDrawable(R.styleable
                .RatingBar_starFill));
        this.clickEnable = mTypedArray.getBoolean(R.styleable.RatingBar_clickEnable, false);
        mTypedArray.recycle();

        paint = new Paint();
        paint.setAntiAlias(true);
        paint.setShader(new BitmapShader(starFillBitmap, BitmapShader.TileMode.CLAMP,
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        {
            starEmptyDrawable.setBounds((starDistance + starSize) * i, 0, (starDistance +
                    starSize) * i + starSize, starSize);
        ...
        if (starMark > 1)
        {
            ...
            else
            {
                ...
                canvas.translate(starDistance + starSize, 0);
                canvas.drawRect(0, 0, starSize * (Math.round((starMark - (int) (starMark)) * 10)
                        * 1.0f / 10), starSize, paint);
            }
        }
        else
        {
